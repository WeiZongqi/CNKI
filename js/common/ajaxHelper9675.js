...
(function () {
    ...
    x.$http = function (opt) {
        ...
        // 用Ajax模拟文件上传：第一步：添加自定义头信息：multi-part/form-data 第二步：就要用FormData，var f=new FormData，f.append("key","value"),当send(f)的时候，当send方法里传formdata是request的content-type会自动变为multi-part/form-data
        //不需要文件上传，单纯的表单提交，mineType是application/x-www-form-urlencoded
